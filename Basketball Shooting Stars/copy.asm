...
ball_motion_manager PROC
	...
	X	mov dumb_var, 150 \ fidiv dumb_var \ fist dumb_var
	X	mov eax, initial_ball_x \ mov ebx, dumb_var \ add eax, ebx \ mov [ball.x], eax

	X	fld shot_dir \ fsin \ fimul shot_power \ fimul time_past
	X	mov dumb_var, 185 \ fidiv dumb_var \ fimul y_dir
	X	fild time_past \ fmul st, st
	X	fidiv gravity \ fsubp st(1), st(0) \ fist dumb_var
